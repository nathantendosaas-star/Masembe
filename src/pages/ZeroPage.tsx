...
const RE_SECTIONS = [
  {
    ...
    desc: "We define the East African skyline through uncompromised precision and visionary property management. Every structure is a testament to our commitment to excellence.",
    accent: "#d4af37"
  },
  {
    title: "Strategic Portfolios",
    desc: "Curating a collection of the region's most prestigious addresses. From high-end residential towers to landmark commercial developments.",
    accent: "#d4af37"
  },
  {
    title: "Bespoke Management",
    desc: "A white-glove approach to asset preservation. We ensure your investments grow in value while maintaining the highest standards of luxury living.",
    accent: "#d4af37"
  }
];
...
export default function ZeroPage() {
  ...
  const reVideoFilter = useTransform(smoothProgress, [0.15, 0.22, 0.45, 0.5], ["blur(10px)", "blur(0px)", "blur(0px)", "blur(10px)"]);
  ...
  const autoVideoFilter = useTransform(smoothProgress, [0.5, 0.57, 0.8, 0.85], ["blur(10px)", "blur(0px)", "blur(0px)", "blur(10px)"]);
    ...
    <div ref={containerRef} className="relative w-full bg-[#F7F7F5] text-black min-h-[800vh] font-sans selection:bg-black selection:text-white">
      ...
      <section className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden z-10">
        <motion.div style={{ opacity: heroOpacity, scale: heroScale }} className="text-center px-6">
          <motion.span className="block text-[10px] md:text-xs font-black tracking-[0.6em] uppercase text-black/30 mb-8">The Collective Intelligence</motion.span>
          <motion.h1 className="text-5xl md:text-8xl lg:text-[10rem] font-black tracking-tighter uppercase leading-none">Masembe<br />Companies</motion.h1>
        </motion.div>
      </section>
      ...
      <div className="relative h-[300vh] w-full z-20">
        <div className="sticky top-0 h-screen w-full flex flex-col md:flex-row items-center overflow-hidden">
          ...
          {/* Sticky Video Side */}
          <motion.div style={{ x: isMobile ? 0 : reVideoX }} className="w-full md:w-1/2 h-1/2 md:h-full relative overflow-hidden">
            <motion.div style={{ scale: reVideoScale, filter: reVideoFilter }} className="w-full h-full">
              <video src="/re-bg.mp4" autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover" />
            </motion.div>
        ...
        <div className="sticky top-0 h-screen w-full flex flex-col-reverse md:flex-row items-center overflow-hidden bg-white">
          ...
          <motion.div style={{ x: isMobile ? 0 : autoVideoX }} className="w-full md:w-1/2 h-1/2 md:h-full relative overflow-hidden">
            <motion.div style={{ scale: autoVideoScale, filter: autoVideoFilter }} className="w-full h-full">
              <video src="/videoplayback.webm" autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover grayscale-[0.3]" />
            </motion.div>
